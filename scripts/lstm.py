...
"""
This script may be used to train and test
LSTMs using the Robobarista dataset. We use
the Keras library to specify our network
architectures. 
...
#Path to where the K-Means models are stored. 
kmeans_path = '/scratch/cluster/rcorona/Robot_Learning_Project/Robot-Learning-Project/Models/Baseline/kmeans_fold_' 

"""
Takes an LSTM result file and uses it to generate
the reference and test files in the format 
required by METEOR for evaluation. 
"""
...
"""
Invokes the Java VM to use METEOR to 
compare the sentences within a reference
and test file that was generated from
LSTM results. 
"""
...
"""
Extracts the METEOR score from 
the file generated by the METEOR
framework during evaluation. 
"""
...
"""
Averages METEOR scores
over all folds for each LSTM
architecture held within the specified folder. 
Prints the results in sorted order. 
"""
def consolidate_meteor_scores(folder_path): 
    meteor_files = {}
    ...
    #Get percentage below METEOR score of 10. 
    print (below_10 / total)

"""
Evaluate the results produced by each
LSTM architecture within the specified 
folder using METEOR. 
"""
...
"""
Read in the specifications of an LSTM
architecture and return a dictionary
containing them. Used to read in the
specs generated by an architecture during
tuning on the validation set in order
...
"""
Given an LSTM specification file (generated during tuning), 
train an LSTM and test it on the test set. Generate results
for all folds and store them in the given results folder. 
"""
...
"""
Randomly generate LSTM architectures 
indefinitely. Each generated architecture
is evaluated on a randomly chosen fold of our dataset.
This is used to tune hyperparameters using the validation set. 
"""
def tune_lstm(results_folder):

    #Keep generating and running LSTM models until manually stopped. 
    while (True): 
...
"""
Print the usage for this script on the command line. 
"""
def print_usage():
...
if __name__ == '__main__':
    if not len(sys.argv) >= 2:
        ...
        if len(sys.argv) == 3: 
            consolidate_meteor_scores(sys.argv[2])            
        else:
            print_usage()
